...
const ProbBars: React.FC<ProbBarsProps> = ({
  probs,
  title = "Model confidence",
}) => {
  const values =
    probs && probs.length === 10 ? probs.map(clamp01) : Array(10).fill(0);
  const topIdx = values.reduce(
        ...
        {values.map((p, i) => {
          ...
          return (
            <div key={i} className="flex flex-row items-center gap-2">
              ...
              <div className="w-4 text-right text-xs tabular-nums text-gray-600 sm:text-right text-left">
                {pct.toFixed(1)}%
              </div>
            </div>
          );
